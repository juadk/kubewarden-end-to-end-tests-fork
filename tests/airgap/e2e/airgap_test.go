...
var _ = Describe("E2E - Build the airgap archive", Label("prepare-archive"), func() {
	It("Execute the script to build the archive", func() {
		...
		GinkgoWriter.Printf("Executed command: %s %s %s %s %s\n", airgapBuildScript, k3sVersion, certManagerVersion, rancherChannel, rancherVersion)
		out, err := exec.Command(airgapBuildScript, k3sVersion, certManagerVersion, rancherChannel, rancherVersion).CombinedOutput()
